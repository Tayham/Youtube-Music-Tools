...
def main_menu() -> None:
    return menu3.Menu(True).menu(title=MENU_TITLE, choices=[REMOVE_LIKED_SONGS_FROM_PLAYLIST_OPTION, REPLACE_UPLOADED_SONGS_WITH_STREAMING_VERSIONS], prompt=MENU_PROMPT_QUIT)

def list_selection_menu(title: str, choices: list[str], prompt: str = MENU_PROMPT, allow_quit: bool = False, allow_skip: bool = False) -> int:
    ...
    if selection_index == -1 and not allow_skip:
        return list_selection_menu(title,choices,prompt,allow_quit,allow_skip)
    else:
